...
public:
    ...
    friend NrComplex SumaNoua(VectC); //FUNCTIA SUMA CE A FOST TRANSFORMATA IN FUNCTIE FREIND
    ...
    VectC operator+(VectC A);
    VectC operator-();
    friend ostream& operator<<(ostream&,NVectC& A);
    friend istream& operator>>(istream&,NVectC& A);
    friend VectC operator--(VectC); //opeator -- pentru obiect
    VectC(bool d);
    bool operator!=(VectC A);
};

NrComplex SumaNoua(VectC A) //functie ce a fost transformata in friend
{
NrComplex New(0,0);
...
int nr;
    public:
        ...
        friend ostream& operator<<(ostream&,NVectC& A);
        friend istream& operator>>(istream&,NVectC& A);
        NVectC operator=(const NVectC&);
        NVectC(const NVectC&);
};

NVectC NVectC::operator=(const NVectC& A) // = pentru vectorul de obiecte
{
nr=A.nr;
...
NVectC::NVectC(const NVectC& A) // constructor de copiere pentru vectorul de obiecte
{
nr=A.nr;
vect = new VectC[A.nr];
for (int i=0;i<nr;i++)
...
ostream& operator<<(ostream& out,NVectC& A) // << pentru vector de obiecte
{
for (int i=0;i<A.nr;i++)
    out<<A.vect[i];
out<<endl;
...
istream& operator>>(istream& in ,NVectC& A) // >> pentru vector de obiecte
{
in>>A.nr;
for (int i=0;i<A.nr;i++)
    in>>A.vect[i];
...
/*int main()
{
    ...
    while (input!='X')
    {
     ...
     switch (input)
        {
        case 'a':
            Sort(A); //sorteaza un vector
            break;
        case 'b':
            Show(A);    //afiseaza un vector
            break;
        case 'c':
            Add(A);     //adauga un vector
            break;
        case 'd':
            Modul(A);      //afiseaza vectorul de module al vectorului ales
            break;
        case 'e':
            Suma(A);    //afiseaza suma elementelor unui vector
            break;
        case 'f':
            ProdusScalar(A);    //afiseaza produsul scalar a doi vectori alesi
            break;
        case 'g':
            A.ShowAll();    //afiseaza toti vectorii
            break;
        }
    }
    VectC A;
...
VectC VectC::operator+(VectC A) // operator + pentru vector complex
{
VectC New(numElem);
for (int i=0;i<numElem;i++)
    New.v[i]=v[i]+A.v[i];
...
VectC VectC::operator-() //operator - pt vector complex
{
for (int i=0;i<numElem;i++)
    {
    v[i].pR=(-1)*v[i].pR;
...
VectC::VectC(bool d) // constructor ce primeste bool
{
if (d==true)
    {
        cout<<"Introduceti numarul de elemente:";
...
bool VectC::operator!=(VectC A) //operator !=
{
if (numElem!=A.numElem)
    return false;
for (int i=0;i<numElem;i++)
    {
    if (v[i]==A.v[i])
